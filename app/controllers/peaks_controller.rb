class PeaksController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @peak.save
        ...
        format.json { render :show, status: :created, location: @peak }
      else
  ...
  def update
    respond_to do |format|
      if @peak.update(peak_params)
        ...
        format.json { render :show, status: :ok, location: @peak }
      else
  ...
  def destroy
    @peak.destroy
    respond_to do |format|
      format.html { redirect_to peaks_url, notice: 'Peak was successfully destroyed.' }
  ...
  private
    ...
    def peak_params
      params.require(:peak).permit(:peaksign, :peakformula, :peakimage, :participation_id)
    end
end
